...
From the output above, we see that the best model (relative to AIC) between `m1` and `m2` is the submodel when removing the interaction term `y:z`. That is, using compact notation
...
The task emphasizes heavily on the relationship between child survival and smoking habits. For simplicity, we chose to focus on the independent effects of each categorical variable. That is, we exclude all interaction in our model such that
...
The corresponding log-linear model is $(XY, XZ, XV, YZ, YV, ZV)$, where the two-way interactions `x:v`, `y:v`, `z:v` are of main interest. Thus, we generate a table a table that includes the estimates and standard error for these three interaction terms.

```{r, echo = FALSE}
m_loglin <- glm(n ~ x*y + x*z + x*v + y*z + y*v + z*v, family = poisson(link = "log"), data = data3)
...
loglinear_df <- data.frame(
  ...
  Coefficient = round(loglinear_coefficients[, "Estimate"], digits = 4),
  Std_Error = round(loglinear_coefficients[, "Std. Error"], digits = 4)
)

# Subset the interactions of interest (x:v, y:v, z:v)
loglinear_interactions <- loglinear_df[grep("x:v|y:v|z:v", loglinear_df$Interaction_Term), ]
