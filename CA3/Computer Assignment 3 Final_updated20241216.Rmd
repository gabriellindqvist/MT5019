...
summary(mbest)
```
##### Significant Two-Way Interactions Based on the Summary (odds ratio aspect following in the next session):


Smoking (\(y\)), gestational age (\(z\)), and child survival (\(v\)), Mother's age (\(x\))
...
```{r, echo = FALSE}
# Extract the coef of interaction items
interaction_terms <- grep(":", names(coef(mbest)), value = TRUE)

interaction_coef <- coef(mbest)[interaction_terms]

# Compute Odds Ratios
interaction_exp_coef <- exp(interaction_coef)

# Compute Confidence Intervals
interaction_confint_vals <- confint(mbest)[interaction_terms, ]
interaction_exp_confint <- exp(interaction_confint_vals)

# Create output table
interaction_results <- data.frame(
  Term = interaction_terms,
  Odds_Ratio = interaction_exp_coef,
  CI_Lower = interaction_exp_confint[, 1],
  CI_Upper = interaction_exp_confint[, 2]
)
...
1. **\(x:y\):** Mother's age is inversely associated with smoking. **(Odds Ratio: 0.6628)** Mothers with higher age are approximately 34% less likely to smoke compared to younger mothers, as indicated by the odds ratio being below 1.

2. **\(x:v\):** Mother's age is positively associated with child survival. **(Odds Ratio: 0.6283)** Older mothers have about a 37% lower odds of experiencing adverse child survival outcomes compared to younger mothers, reflecting a protective effect of maternal age.

3. **\(z:v\):** Gestational age is strongly positively associated with child survival. **(Odds Ratio: 27.4220)** With every unit increase in gestational age, the odds of child survival increase more than 27 times, highlighting a highly significant and strong positive relationship.

4. And we can see that x:z(Mother's age : Gestational age) and y:v(Smoking : Child survival) are not significant since their 95% CI include the value 1.
